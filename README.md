...
为了尽量避免“不同 loop 实际落到同一个真实后端会话里”，官网客服模式现在默认：

- 不透传原始抓包里的 `Cookie`

这样做的目的，是尽量让每个测试 loop 都重新初始化一段新的官网客服会话，减少旧历史污染。

另外，官网客服常见的首轮营销欢迎语现在会被单独识别并记录：

- 会写入日志和报告
- 但不会被当成 SDR 的正式回复推进下一轮模拟对话
- 如果当前只收到欢迎语、还没收到真正业务回复，工具会继续等待正式回复

如果你确实需要复用原网页会话，可以在配置里显式打开：

```json
{
  "preserveWebsiteCookies": true
}
```

如果后端仍然出现跨 loop 复用同一个真实 `sessionId`，报告里的 `关键发现` 会直接提示“会话未隔离风险”。
